...
import { createInstructorProposal } from '@/lib/instructor-portal/actions';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await createInstructorProposal(input);
    return NextResponse.json({ success: true, ...result });
  } catch (err) {
    console.error('[POST /api/instructor/courses] error:', err);
