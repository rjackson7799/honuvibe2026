...
| 5 | Nav + Footer cutover (UserMenu integration polish, mobile menu polish) | ✅ done | 2026-04-27 — type-check ✓, 338 vitest tests ✓, footer reconciled |
| 6 | Cleanup: redirects, retire dead routes, prune `components/sections/*`, restore `<main>` padding | ✅ done | 2026-04-27 — type-check ✓, 311 vitest tests ✓ (30 files), build ✓ (378 static pages) |
...
| 2026-04-27 | 5 | `pnpm type-check` ✓ · 338 vitest tests ✓ (35 files, 27s) | cc045bc (footer + tests) + plan-doc commit | Phase 5 cutover landed. Most of the scope (auth-aware MarketingNav, MarketingMobileMenu, hamburger drawer, Honuhub-removed-from-nav) had already shipped during the Phase 0/4 component commits — verified in place. Phase 5 narrowed to: (a) footer reconciliation (drop `/honuhub` + `/community` from Navigate column; rewrite `/newsletter` link to `/#newsletter` anchor since Phase 6 redirects there); (b) new `__tests__/marketing/marketing-user-menu.test.tsx` covering signed-out / signed-in non-admin / signed-in admin branches via a Supabase client mock (4 tests). **Divergence from umbrella plan:** the Phase 5 row originally called for threading `variant="marketing"` through legacy `components/layout/user-menu.tsx` + `lang-toggle.tsx`. Reality shipped parallel marketing-namespaced components (`MarketingUserMenu`, `MarketingLangToggle`) styled directly against `--m-*` tokens. Divergence accepted; no consolidation planned. Plan: `docs/plans/2026-04-27-phase-5-nav-footer-cutover.md`. |
...
| 2026-04-27 | 6 | `pnpm type-check` ✓ · 311 vitest tests ✓ (30 files, 28s) · `pnpm next build` ✓ (75s compile, 378 static pages) | 3d5c05e (commit A) + commit B | Phase 6 cleanup landed end-to-end. **Commit A** (`3d5c05e`): added 12 new permanent (308) redirects (`/build`→`/explore`, `/community`→`/about#aloha`, `/resources`→`/learn`, `/newsletter` and `/newsletter/:slug*`→`/#newsletter`, `/become-an-instructor`→`/partnerships`, all with `/ja/...` variants), rewrote 9 existing redirect destinations so no chain points at a deleted route (`/tools, /toolkit, /stack, /emails, /archive` + `/ja/...` variants now terminate directly at `/learn` or `/#newsletter`). Deleted 5 route directories (`app/[locale]/{build,community,resources,newsletter,become-an-instructor}/`) including `newsletter/[slug]/` archive. Wide component sweep: 11 top-level `components/sections/*.tsx` files (kept `cta-strip.tsx` — still used by `/glossary`), all of `components/sections/{community,about,contact,learn}/`, all of `components/sections/exploration/` except `tech-icon.tsx` (used by `lib/tech-items.tsx` + `components/partners/vertice-page-content.tsx`). Sibling dirs `components/{build,resources,newsletter}/` deleted entirely (10+5+4 files); 6 corresponding test files in `__tests__/components/{resources,newsletter}/` deleted. The `components/sections/index.ts` barrel removed. **Commit B**: removed the `-mt-14 md:-mt-16` compensation hack from `MarketingShell` (now ships clean `min-h-screen`); widened `ConditionalMain`'s no-padding decision in `components/layout/conditional-nav.tsx` to cover both auth-shell AND marketing routes via `isMarketingPathWithLocale` — legacy public surface (`/honuhub, /blog, /glossary, /privacy, /terms, /cookies`) keeps `pt-14 md:pt-16` to clear the dark Nav. `__tests__/marketing/shell.test.tsx` flipped to assert `not.toContain('-mt-')`; new regression test file `__tests__/marketing/conditional-main.test.tsx` (19 it.each cases) guards the padding gate against regressions across marketing/auth/legacy routes. **HonuCompanion** gate already in place at `components/ocean/honu-companion.tsx:45` (likely landed Phase 0 alongside `lib/marketing-routes.ts`) — verified, no code change. **Net:** 76 files in commit A (-6315 deletions, +214 insertions) + ~6 files in commit B. **Deferred:** i18n namespace prune — investigation revealed the `newsletter` top-level namespace is still consumed by `marketing-newsletter.tsx` + `blog/newsletter-cta.tsx`, and other candidates turned out to be sub-keys nested in live namespaces. Focused i18n audit is a low-risk follow-up. **Known follow-ups** (out of scope, click-through works via redirects): legacy `components/layout/{nav,footer}.tsx` still link to `/build, /community, /newsletter`; `components/partners/smashhaus/SmashHausLanding.tsx:895` has `<Link href="/build">`. Plan: `docs/plans/2026-04-27-phase-6-cleanup.md`. |
