-- Fix: Add course_id column to course_sessions for bonus session support.
-- Bonus sessions have week_id = NULL, so they need a direct course reference.
--
-- NOTE: migration 016_bonus_sessions.sql already adds this column. This file
-- exists as a defensive fix for envs where 016 partially applied. Made
-- idempotent so fresh local boots (where 016 fully applied) don't error.

-- 1. Add the column (nullable first for backfill) — idempotent
ALTER TABLE course_sessions ADD COLUMN IF NOT EXISTS course_id uuid REFERENCES courses(id) ON DELETE CASCADE;

-- 2. Backfill from existing curriculum sessions via their week
UPDATE course_sessions
SET course_id = (SELECT course_id FROM course_weeks WHERE id = course_sessions.week_id)
WHERE week_id IS NOT NULL AND course_id IS NULL;

-- 3. Make it NOT NULL now that all rows are populated — idempotent via DO block
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'course_sessions'
      AND column_name = 'course_id'
      AND is_nullable = 'YES'
  ) THEN
    ALTER TABLE course_sessions ALTER COLUMN course_id SET NOT NULL;
  END IF;
END $$;

-- 4. Create the index that migration 016 intended
CREATE INDEX IF NOT EXISTS idx_course_sessions_bonus ON course_sessions (course_id, is_bonus, scheduled_at);
