...
// Instructor-side view: student identity hidden by design (per Gap 2 deferral)
type OwnPayoutFilters = Pick<InstructorPayoutFilters, 'status' | 'from' | 'to'>;

export async function getOwnInstructorPayouts(
  filters: OwnPayoutFilters,
): Promise<InstructorPayoutSummary> {
  ...
  if (error) {
    console.error('[Revenue Split] Failed to load own instructor payouts:', error);
    return {
      filters: { ...filters, instructorId: null },
      rows: [],
      totalsByCurrency: {},
    };
  }

  type OwnRawPayoutRow = Omit<RawPayoutRow, 'enrollments'> & {
    enrollments: {
      amount_paid: number | null;
      courses: { title_en: string | null } | null;
    } | null;
  ...
  const rows = ((data ?? []) as unknown as OwnRawPayoutRow[]).map((row) => ({
    id: row.id,
    enrollment_id: row.enrollment_id,
    instructor_id: row.instructor_id,
    instructor_display_name: row.instructor_profiles?.display_name ?? null,
